...
Case 03 is a real finite-wing vortex-lattice solve. `vlm-core.js` is a clean-room browser implementation of the Katz-Plotkin quarter-chord / three-quarter-chord formulation used by Aeronauty's earlier browser circulation lab. The steady calibration keeps the one-row Weissinger-L comparison: cosine span spacing, a straight Kutta wake, solved lift, an independent Prandtl lifting-line result, the no-penetration residual and trailing-sheet closure.

The harmonic mode is a time-accurate finite-wing UVLM, not the old post-processed roll-up picture. The fixed body-frame wing undergoes pure-heave forcing through `wN = -hDot`. At every step its unknown trailing vortex-ring row closes at `beta U dt` behind the physical trailing edge (`beta = 0.25`). That line is retained; at the next step it is connected to the new line with the previous trailing-edge ring strength. Every shed ring keeps its circulation and remains in the solution for all four cycles. The three treatments share the same forcing: flat mean-plane wake, prescribed trailing-edge history, and free near-wake roll-up. In the free treatment, all rows influence the wing and near wake, while only the newest 12 rows receive induced-motion corrections; the older rows continue to convect in the body frame. This makes a multi-cycle browser solve practical without deleting circulation history.

The Case 03 applet replays 24 phase snapshots for every wake treatment, so switching between flat, TE-history and self-induced wake geometry never launches a different solve. The moving-wing and body-fixed controls are also views of one numerical state. In the moving view the wing follows `h(t)`; the TE-history and self-induced wakes are transformed back to inertial coordinates, while the linearized flat wake remains on its prescribed mean plane. Up to 48 indexed rows preserve the complete near wake and span the entire retained wake age, always including the oldest row. Whole-wake framing may compress only the displayed streamwise coordinate, and the renderer states that factor explicitly. No such magnification or compression enters the solver.

The self-induced view overlays the same-phase TE-history wake as a dashed ghost and reports the maximum physical separation before display magnification. This makes the additional induced motion directly inspectable instead of asking the reader to infer it from a label. A signed lift arrow, the large `CL,circ(t)` value, and the chart cursor all use the selected stage's same instantaneous pressure-derived circulatory-load sample.

Loads follow the discrete unsteady pressure relation. The worker fits the instantaneous pressure and potential terms over complete cycles, evaluates the derivative in phasor space, subtracts a no-wake added-mass calibration built on the same `c + beta U dt` harmonic closure, and normalises by the same wing's steady VLM lift slope. The plotted time trace removes that same analytic added-mass term, so it represents the reported pressure-derived circulatory load rather than bound circulation alone. Magnitude and phase are published only when the last two cycles satisfy the periodicity, harmonic-fit and normalised residual gates. The reader reports retained loading, lift deficiency, lag, wake age, active/free rows, core radius, no-penetration, TE shedding and filament-continuity residuals. The full formulation, approximations and test matrix are in [`UVLM-NUMERICAL-NOTE.md`](./UVLM-NUMERICAL-NOTE.md).
